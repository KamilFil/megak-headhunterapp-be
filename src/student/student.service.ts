  ...
  async updateStudentUser(id: string, updatedStudent: StudentUser) {
    ...
    if (emails.filter((email) => email === updatedStudent.email).length) {
      throw new ValidationError(
        `${updatedStudent.email}: email already exist.`,
      );
    }
    ...
    const gitHubUser = await axios
      .get(`https://api.github.com/users/${updatedStudent.githubUsername}`)
      .then((res) => res.data)
      .catch((err) => err.data);

    if (!gitHubUser) {
      throw new ValidationError(
        `No such GitHub user as: ${updatedStudent.githubUsername}`,
      );
    }

    if (
      gitHubUsernames.filter(
        (username) => username === updatedStudent.githubUsername,
      ).length
    ) {
      throw new ValidationError(
        `${updatedStudent.githubUsername}: gitHubUsername already exist.`,
      );
    }
